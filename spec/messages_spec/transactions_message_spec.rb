...
describe 'TransactionsMessage' do
  context 'when initialised' do
    let(:transaction_1) { double :transaction, type: :Deposit, date: '1/1/01', amount: 34.89    }
    let(:transaction_2) { double :transaction, type: :Withdrawal, date: '2/2/02', amount: 12.89 }
    let(:transaction_3) { double :transaction, type: :Deposit, date: '3/3/03', amount: 89.89    }
    ...
    it 'has the correct main text' do
      expect(test_message.main).to eq(['1. Type: Deposit, Date: 1/1/01, Amount: £34.89',
                                       '2. Type: Withdrawal, Date: 2/2/02, Amount: £12.89',
                                       '3. Type: Deposit, Date: 3/3/03, Amount: £89.89'])
    end

    it 'has the correct output' do
      expect(test_message.output).to eq(['Transaction Successful.',
