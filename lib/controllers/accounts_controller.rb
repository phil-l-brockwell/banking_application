...
class AccountsController
  ...
  def open_account(type, with:)
    return InvalidHolderMessage.new(with) unless holder = holders.item_exist?(with)
  ...
  def deposit(amount, into:)
    return InvalidAccountMessage.new(into) unless account = item_exist?(into)
    account.deposit amount
    DepositSuccessMessage.new(amount)
  end

  def withdraw(amount, from:)
    return InvalidAccountMessage.new(from) unless account = item_exist?(from)
  ...
  def get_balance_of(account_id)
    return InvalidAccountMessage.new(account_id) unless account = item_exist?(account_id)
    BalanceMessage.new(account)
  end

  def transfer(amount, from:, to:)
    return InvalidAccountMessage.new(from) unless donar = item_exist?(from)
    return InvalidAccountMessage.new(to) unless recipitent = item_exist?(to)
  ...
  def add_holder(id, to_account:)
    return InvalidHolderMessage.new(id) unless new_holder = holders.item_exist?(id)
    return InvalidAccountMessage.new(to_account) unless account = item_exist?(to_account)
